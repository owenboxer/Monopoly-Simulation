...
public class Card {
	...
	public int getQuantity(){ //gets quantity for type 1 cards (and is used by other classes)
		if (repair) {
			quantity = 0;
			for (int p = 0; p < Board.player[Board.currentPlayer].property.size(); p++)
				quantity += (houseRepairCost * Board.player[Board.currentPlayer].property.get(p).houses) +
							(hotelRepairCost * Board.player[Board.currentPlayer].property.get(p).hotels);
		}
		else if (playerModifier != 0)
			quantity = Board.playerQuantity * playerModifier;

		//if neither conditionals are triggered, quantity will already have been set
		return quantity;
	}

	public void performFunction(){
		if (effect == 1) {
			Board.player[Board.currentPlayer].exchangeMoney(getQuantity());
		}
